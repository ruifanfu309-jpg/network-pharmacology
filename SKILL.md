...
- User mentions 网络药理学, 前期物质筛选, OB/DL 筛选, 类药性, 靶点预测, 分子对接, or a target disease/direction — **ANY disease keyword works** (抗氧化 antioxidant, 抗炎 anti-inflammatory, 降糖 hypoglycemic, 护肝 hepatoprotective, 心血管 cardiovascular, 神经保护 neuroprotective, ...). The disease-target stage is CONFIGURABLE (see "Disease direction (可配置)" below) — never hard-code a single disease.
...
- `scripts/step1_structures.py` — batch PubChem name → SMILES lookup (rate-limited, name-cleaning fallback). **Parses `ConnectivitySMILES`** (see Pitfall 1). Outputs `pubchem_smiles.json`.
- `scripts/step2b_tcmsp.py` — batch TCMSP OB/DL/BBB/Caco2/HL lookup (token + `qn=` detail page; verified 2026-08), writes `tcmsp_adme.csv`. Reads `类药性筛选结果.csv`.
...
- `scripts/step5_disease_genes.py` — gene→canonical UniProt mapping (Step 4, `reviewed:true`), writes CSV.
- `scripts/step5_disease_genes.py` — batch UniProt GO-term query (disease-target gene sets; bare `(GO:...)` syntax, verified 2026-08).
- `templates/report_template.html` — force-directed 成分-靶点 network graph template (fill `nodes`/`links` JSON).
...
## Published repository & publishing workflow (2026-08)

## Published repository & publishing workflow (2026-08)

The skill is published as a public GitHub repo (verified live):
- **Repo**: `https://github.com/ruifanfu309-jpg/network-pharmacology` (Public, MIT, 14 files, 1185 lines)
- **Local git checkout**: `Desktop/network-pharmacology-skill/` (git repo, remote `origin` set, branch `main`)
- **Sync after updating the local skill**: copy `AppData/Local/hermes/skills/research/network-pharmacology/*` over the checkout, then `git add -A && git commit -m "..." && git push origin main`. Windows git auto-authenticates via Git Credential Manager when credentials are cached — no token prompt needed.
- **Sanitize before publishing**: the Hermes-side SKILL.md may carry user-specific values. Replace proxy addresses (`<PROXY:PORT>` → `<YOUR_PROXY:PORT>`) and check for user home paths / emails before committing (no API keys were present, but verify each time).
- **New-repo branch gotcha**: fresh `git init` on modern git defaults to `main` — `git push origin main`, NOT `master` (else `error: src refspec master does not match any`).
- **Repo creation via browser automation** (use when the REST API is unreachable): `github.com/new` → fill `#repository-name-input` + description (`#_r_e_`); the visibility control is a CUSTOM widget — `input[type=radio]` count is 0, so click the visible element whose textContent is exactly "Public" (guard with `el.offsetParent !== null`), then click the "Create repository" button. Success = URL redirects to `https://github.com/<user>/<repo>`. Verify afterward by opening the repo page and checking the file tree renders.
- **GitHub web UI language follows the browser's Accept-Language — there is NO in-site language setting** (Settings → Appearance only has theme prefs). To get the Chinese UI: Chrome settings → languages → move 中文(简体) to the top, then RESTART Chrome (restart is required to take effect). Note `chrome://settings/...` pages return EMPTY `body.innerText` to browser_exec (sandboxed internal page) — you cannot drive them; hand the user the click steps instead.
- **Keep the repo README's case-study numbers in sync** with the skill's worked-example figures (README currently quotes 138 metabolites → 774 edges / 78 genes / p=6.65e-97; update both when the pipeline numbers change).

Run with the uv-managed venv python (NOT the Hermes bundled `venv/Scripts/python`):
```bash
"/c/Users/<user>/AppData/Local/hermes/hermes-agent/.venv/Scripts/python.exe" step1_structures.py
```
...
14. **ADMET (absorption/metabolism/toxicity) is a REQUIRED stage this user explicitly wants filled** — TCMSP OB/DL is query-table (32% coverage on MS metabolites) and the user correctly rejects "查不到就不查". The calculation-based route: SwissADME (GI absorption, bioavailability score, solubility) or **ADMET-AI** (local ML model, batch 72+ SMILES in one run: absorption + metabolism + toxicity). Install ADMET-AI with `uv pip install admet-ai` — Tsinghua mirror timed out for this package; use official PyPI behind the VPN proxy (`export HTTPS_PROXY=http://<PROXY:PORT>`). **GOTCHA: a global `uv.toml` with `[pip] index-url = <mirror>` OVERRIDES the HTTPS_PROXY env var — uv keeps hitting the dead mirror. Delete `%APPDATA%\uv\uv.toml` (or run without it) before installing; re-create it afterward if needed.** ~1GB deps (torch); CPU inference is fine for a few hundred compounds. Use its outputs (GI absorption, toxicity classes) as a soft filter alongside Lipinski — same logic as OB/DL: flag Low-absorption/High-toxicity compounds, don't hard-delete on a single prediction. **ADMET-AI v2 output columns DIFFER from the docs** — verify with `avail = [c for c in key_cols if c in preds.columns]` and dump `sorted(preds.columns)` once. Verified real names (2026-08): `Bioavailability_Ma` (absorption), `Caco2_Wang` (gut permeability), `BBB_Martins`, `CYP1A2_Veith`/`CYP2C9_Veith`/`CYP2D6_Veith`/`CYP3A4_Veith`/`CYP2C19_Veith` (metabolism), `CYP2C9_Substrate_CarbonMangels` etc., `Clearance_Hepatocyte_AZ`, `hERG` (cardiotox), `AMES` (mutagenicity), `Carcinogens_Lagunin` (carcinogenicity), `Pgp_inhibitor`. ~104 columns total; pick ~15. Screening rule used: Bioavailability > 0.5 green, AMES < 0.5 green.
...
```python
POST https://biit.cs.ut.ee/gprofiler/api/gost/profile/   # proxy <PROXY:PORT>
...
- **Disease target gene→UniProt mapping** (needed for network nodes):
  ...
  **Must include `reviewed:true`** or you get random isoforms (A0A... accessions) instead of canonical Swiss-Prot entries. When parsing a gene-list text file, strip everything after `#` (inline Chinese comments become garbage tokens otherwise). Script: `scripts/step5_disease_genes.py`.
...
- Clicking the "100 per page" pagination control has NO effect in automation (DataTables widget) — but a HUMAN user CAN read the full Top-50 manually in their own browser session (verified 2026-08). Default top-20 UNDERSHOOTS (misses ALOX5 #34, IDO1, SLC7A11...); get the user's Top-50 list and merge it with the GO-term gene set (see V5).
...
Replaces manual gene-list curation — pull ALL reviewed human genes annotated to disease-relevant GO terms. **Disease direction is a parameter, not a constant:**
...
| 心血管 cardiovascular | GO:0008015 blood circulation, GO:0008217 blood pressure regulation, GO:0048514 blood vessel morphogenesis |
| 神经保护 neuroprotective | GO:0043523 neuron apoptotic process regulation, GO:0050804 synaptic transmission modulation |

⚠️ **Direction check (user's 2026-08 lesson)**: GeneCards keyword search is TEXT-MINING — for `antioxidant` it returned ALOX5/BRAF (pro-inflammatory/kinase, OPPOSITE direction). If the user wants a PURE disease set, use GO annotations ONLY (no GeneCards merge); if they accept related genes, keep GeneCards top-N and note the caveat. When in doubt, ASK.
